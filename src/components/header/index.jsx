...
const Header = () => {
  ...
  let sum = 0
  reserveList.map((room)=>{sum += room.count})

  const isLogin = localStorage.getItem("isLogin");
  return (
    <header className="p-2 px-10 shadow-xl bg-sky-900 opacity-[0.9] flex items-center justify-between gap-[100px] sticky z-50 top-0 w-[full]">
      <img src={logo} alt="logo" className="w-[100px]" />
      <ul className="flex gap-20 font-semibold text-sky-300 uppercase">
        <li>
          <NavLink
            to={HOME_ROUTE}
            className={({ isActive }) =>
